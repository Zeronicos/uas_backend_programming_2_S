            ...
            <div class="row">
                                    ...
                                    @foreach (Cart::content() as $product)
                                        <tr>
                                            ...
                                            <td class="fp__pro_name">
                                                ...
                                                <span>{{ @$product->options->product_option['name'] }}
                                                    {{ @$product->options->product_option['price'] ? '(' . currencyPosition(@$product->options->product_option['price']) . ')' : '' }}</span>
                                                @foreach ($product->options->product_add as $add)
                                                    <p>{{ $add['name'] }} ({{ currencyPosition($add['price']) }})</p>
                                                @endforeach
                                            </td>
                                    ...
                                    @if (Cart::content()->count() === 0)
                                        <tr>
                                            <td colspan="6" class="text-center fp__pro_name"
                                                style="width: 100%; display: inline; color: #D9D9D9">Cart is empty!</td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
                ...
                <div class="col-lg-4 wow fadeInUp" data-wow-duration="1s">
                    <div class="fp__cart_list_footer_button">
                        ...
                        <p class="total">
                            <span>total:</span> <span id="final_total">
                            @if (isset(session()->get('coupon')['discount']))
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let couponCartHtml = `<div class="card mt-2">
                                        <div class="m-3">
                                            <span><b class="v_coupon_code">Used Coupon: ${response.code}</b></span>
                                            <span>
                                                <button id="destroy_coupon"><i class="far fa-times"></i></button>
                                            </span>
                                        </div>
